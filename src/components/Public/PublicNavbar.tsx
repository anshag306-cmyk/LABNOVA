      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            {onVerifyReportClick && (
              <button
                id="btn-nav-verify-report"
                type="button"
                onClick={onVerifyReportClick}
                className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition shadow-xs"
              >
            ...
            {/* Book Home Collection Quick Button */}
            <button
              id="btn-nav-book-phlebotomy"
              type="button"
              onClick={onOpenHomeBooking}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white text-xs font-bold shadow-sm transition active:scale-95"
            >
            ...
            {/* If Not Authenticated: Login / Sign In button */}
            {!isAuthenticated && handleLoginClick && (
              <button
                id="btn-nav-login-signin"
                type="button"
                onClick={handleLoginClick}
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold shadow-xs transition"
              >
                <Lock className="w-3.5 h-3.5 text-blue-600 dark:text-teal-400" />
                <span>Login / Sign In</span>
              </button>
            )}

            {/* If Authenticated: Access to Dashboard */}
            {isAuthenticated && user && (
              <div className="flex items-center gap-2 pl-2 border-l border-slate-200 dark:border-slate-800">
                <button
                  id="btn-nav-authenticated-dashboard"
                  onClick={onGoToDashboard}
                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-teal-600 hover:from-blue-700 hover:to-teal-700 text-white text-xs font-bold shadow-sm transition-all transform active:scale-95"
                >
                  <Building2 className="w-4 h-4" />
                  <span className="hidden sm:inline">LIMS Workspace</span>
